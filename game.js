...
function loadImages() {
  function loadImage(imgName, index) {
    var img       = window[imgName + "Image"],
        ...
        imgPrefix = imgName.indexOf("badger") >= 0 ||
                    imgName === "exec" ? ".png" : ".jpg";

    window[imgName + "Image"] = new Image();
    window[imgName + "Image"].src = "assets/" + imgName + imgPrefix;

    window[imgName + "Image"].onload = function() {
      window[imgName + "Ready"] = true;
    }
  }
...
render = function () {
  ...
  if (execReady) {
    context.drawImage(execImage, exec.x, exec.y);
  }
